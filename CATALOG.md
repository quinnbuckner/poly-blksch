...
- **Apr 23 2026 — Stage-0 §6 gate chase.** 4 commits from `1b4d7bf` (initial failing gate, 2353 MSE vs 70 target) through `f257731` (PASS at 75.09). Intermediate: `ac23339` (BV warm-start in `em_calibrate(initial_params=None)`), `be00bc5` (`filter/ewma_var.py` — production σ̂_b²(u) forecast, H=90s sweep-selected). **Root cause:** test was computing realized variance on true synthetic `path.x` instead of filtered `x_hat` per paper §6.1. One-line fix at f257731 flipped EWMA↔RV correlation from −0.15 to +0.77 and pulled MSE in-band (+6.8%).

- **Apr 23 2026 — Stage-1 scaffold.** Three parallel branches: `d395f20` (app.py full asyncio wire — screener → polyclient → canonical_mid → Kalman → background em_calibrate → RefreshLoop → paper_engine), `4eea13e` (snapshot regressions, 25 golden fixtures at 1e-10 tolerance locking all merged modules), `3b8371d` (live-recorded contract fixtures + `scripts/record_contract_fixtures.py`, 3 polyclient contract tests auto-activated), `d0e10a8` (`scripts/calibration_dryrun.py` — per-market GREEN/YELLOW/RED GO/NO-GO gate, 822 LOC).

- **Apr 23 2026 — Pre-soak audit rounds 1 + 2 (11 bugs).** Two Explore/general-purpose fleets found 11 latent bugs across app.py, calibration_dryrun, paper_soak. Round-1 fixes: `3ab856b` (app.py screener leak + early-exit ledger-close + config KeyError), `f511e49` (calibration_dryrun ParquetStore leak + --log-level + WS async-gen close), `94e39c0` (app.py WS async-gen symmetric fix). Round-2: `97042c8` (paper_soak stdout PIPE deadlock + silent sampler + hour-boundary carryover), `b393702` (paper_soak --market→--tokens CLI + SIGINT artifact loss + exit-code contract + hours_observed inflation + rc=0 log level). Plus `43c4c81` (sticky kill-switch pause per paper §4.6 — `mm/limits.py` + `mm/refresh_loop.py`, single source of truth, operator-only resume).

- **Apr 23 2026 — Multi-seed gate robustness (`fac7d13`).** Gate converted from single-seed point-estimate to median-of-5-seeds assertion. 15× max-MSE ceiling documents the known KF→UKF blend boundary pathology at \|x\|>4.5 (see `memory/project_boundary_regime_em_inflation.md`); tightening to 3× deferred until boundary-regime kalman fix lands.

- **Apr 24 2026 — Phase-1a pre-soak doc + config refresh (`cc5507e`).** CATALOG stage tracker + 4 module-status rows flipped ⬜→✅ (`ewma_var`, `surface/smooth`, `surface/corr`, `diagnostics`); README removed stale "not wired yet"; `scripts/README.md` gained 5 new entries (calibration_dryrun, paper_soak, reconcile_drill, record_contract_fixtures, live_ro_auth_check); `config/bot.yaml::limits.feed_gap_sec` 3→10 for residential hotspot tolerance (restore to 3 for cloud Stage-2); `.gitignore` excludes `soak_output/` + `.claude/`.

- **Apr 24 2026 — Track C Stage-2-prep-small (`51c6884`).** 3 operational additions: `scripts/paper_soak.py --min-hours FLOAT` (decouple acceptance duration floor from `--hours`), `--soak-output-dir PATH` (prevent parallel soaks from clobbering artifacts), `src/blksch/exec/dashboard.py` honors `BLKSCH_DASHBOARD_PORT` env var via new `resolve_dashboard_port()` helper. +3 tests.

- **Apr 24 2026 — Track C fit_k_from_ledger (`257fad8`).** `scripts/fit_k_from_ledger.py` (680 LOC) — post-soak Avellaneda-Stoikov `k` recovery. Opens `exec/ledger.db` read-only (SQLite `mode=ro` URI), extracts paired orders/fills within `--pair-window-sec`, computes δ in logit units, runs **Poisson-weighted** log-linear regression (`np.polyfit(deg=1, w=counts)`) — naive unweighted was 30%+ off on synthetic data. Outputs JSON with realized k, 95% CI, R², `suggested_k`, `should_retune` flag. 5 unit tests.

- **Apr 24 2026 — Track B live-mode CLOB cleanup guard (`258eac0`).** `src/blksch/app.py` live-mode branch now wraps `CLOBConfig.from_env + make_clob_client` in try/except routing through `_cleanup(client if owned_client else None, ledger if owned_ledger else None)` then re-raises. Mirrors the screener-leak fix pattern from 3ab856b. Pre-Stage-2 ownership-contract hardening. +3 integration tests.

- **Apr 24 2026 — Track A polyclient mock-mode routing (`4166301`).** `MockPolyClient` subclass with deterministic GBM stream (σ_b=0.02 logit drift, 250ms interval, 10% trade probability, `MOCK_POLYCLIENT_SEED` env seed); transparent routing via `is_mock_token()` on prefixes `0xmock` / `mock:` (case-insensitive). `stream_market` + `get_book` both honor the mock. Mixing mock and real token_ids in one call raises ValueError. Paper-soak rehearsal now shows `min_quote_uptime_pct: 100.0` (was 0.0 pre-fix) because pipeline fully exercises without network. **Fast suite at `4166301`: 1102 passed / 15 skipped / 0 failed.**
